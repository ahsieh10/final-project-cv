...
def get_embedding(person: Person):
  keypoint_arr = np.zeros([17,2])
  for keypoint in person.keypoints:
    keypoint_arr[keypoint.body_part.value, 0] = keypoint.coordinate.x
    keypoint_arr[keypoint.body_part.value, 1] = keypoint.coordinate.y
  
  hip_center_x = (keypoint_arr[11,0] + keypoint_arr[12,0]) / 2
  hip_center_y = (keypoint_arr[11,1] + keypoint_arr[12,1]) / 2

  shoulder_center_x = (keypoint_arr[5,0] + keypoint_arr[6,0]) / 2
  shoulder_center_y = (keypoint_arr[5,1] + keypoint_arr[6,1]) / 2

  body_size = ((hip_center_x - shoulder_center_x)**2 + (hip_center_y - shoulder_center_y)**2)**(1/2)

  body_center = np.array([[hip_center_x, hip_center_y]])

  keypoint_arr = keypoint_arr - body_center

  keypoint_arr /= body_size

  return keypoint_arr.flatten()
