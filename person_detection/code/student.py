...
def get_tiny_images(image_paths, extra_credit=False):
    ...
    output = np.zeros((num_images, 256))
    for i in range(num_images):
        img = skimage.io.imread(image_paths[i])
        tiny_img = skimage.transform.resize(img, (16,16), anti_aliasing=True)
        row_tiny_img = np.reshape(tiny_img, (1, 256))
        output[i] = row_tiny_img
    return output
...
def build_vocabulary(image_paths, vocab_size, extra_credit=False):
    ...
    kmeans = sklearn.cluster.KMeans(n_clusters=vocab_size, max_iter=100)
...
def get_bags_of_words(image_paths, vocab, extra_credit=False):
    ...
    all_histograms= np.zeros((1, len(vocab)))
    for i in range(num_imgs):
        ...
        # build histogram for each image's feature_descriptors
        histogram = np.zeros((1, len(vocab)))
        distance_matrix = scipy.spatial.distance.cdist(feature_descriptor, vocab, 'euclidean')
        sort_distance_indices = np.argsort(distance_matrix, axis=1)
        closest_indices = sort_distance_indices[:,0:1]
        closest_indices = closest_indices.flatten()

        for j in range(len(closest_indices)):
            histogram[0, closest_indices[j]] += 1

        histogram = histogram / np.linalg.norm(histogram)
        all_histograms = np.concatenate((all_histograms, histogram), axis=0)
...
def svm_classify(train_image_feats, train_labels, test_image_feats, extra_credit=False):
    ...
    num_imgs = len(test_image_feats)
    k = len(classes)
 
    scores = np.zeros((k, num_imgs))
    for j in range(k):
        curr_label = classes[j]
        train_label = train_labels.copy()
        A = np.where(train_label == curr_label)
        B = np.where(train_label != curr_label)
        train_label[A] = 1
        train_label[B] = -1

        weights, bias = SVM().train(train_image_feats, train_label)
        test_score = (np.matmul(weights, np.transpose(test_image_feats)) + bias).flatten() 
        scores[j] = test_score

    max = np.argmax(scores, axis=0)
    final_labels = classes[max]
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats, extra_credit=False):
    ...
    num_test_images = len(test_image_feats)

    distance_matrix = scipy.spatial.distance.cdist(test_image_feats, train_image_feats, 'euclidean')
    sort_distance_indices = np.argsort(distance_matrix, axis=1) 
    train_labels = np.array(train_labels)

    closest_indices = sort_distance_indices[:,0:k]

    # for each test image feature ...
    labels = []
    for i in range(num_test_images):
        closest_train_indices = closest_indices[i]
        k_labels = train_labels[(closest_train_indices)]
        most_common_label = (scipy.stats.mode(k_labels, axis=None, keepdims=False))[0]
